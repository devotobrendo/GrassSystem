...
### Fixed
- **Ground Decal Rendering** - Fixed issue in ground decal shader render queue layer.
...
### Fixed
- **Critical: White Grass After Update** — `shader_feature_local` for color modes (`_COLORMODE_ALBEDO/_TINT/_PATTERNS`) and `_LIGHTPROBES_ON` was stripping shader variants because keywords are set at runtime via `materialInstance.EnableKeyword()`, not on the Material asset. Changed to `multi_compile_local` (same fix already applied to `_DECALS_ON` in v4.3.1). Affects both GrassLit and GrassUnlit shaders.
